...
class AdvancedViewController: UIViewController {
  ...
  func video1(){
    do {
      ...
      guard let path = Bundle.main.path(forResource: "bridge", ofType: "mp4") else {return }
      guard let path2 = Bundle.main.path(forResource: "crunches", ofType: "mp4") else {return }
  ...
  //fungsi start
  func start(){
    timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(AdvancedViewController.updateTimer), userInfo: nil, repeats: true)
  }
  ...
  func alert(){
    ...
    alert.addAction(UIAlertAction(title: "Next", style: .destructive , handler: {  action in
      ...
      self.performSegue(withIdentifier: "toRestIntermediate", sender: nil)
    }))
  ...
  func showAlert(){
    ...
    alert.addAction(UIAlertAction(title: "Yes", style: .destructive , handler: {  action in
      self.performSegue(withIdentifier: "homeInter", sender: nil)
    }))
    present(alert, animated: true)
  }
